...
NATURAL_DEPRESSION_PAN_INDIA_LOCAL_PATH = (
    PROJECT_ROOT / "data/base_layers/Pan_India_natural_depression.tif"
)
NATURAL_DEPRESSION_OUTPUT_BASE_DIR = PROJECT_ROOT / "data/layers/natural_depression"
...
def generate_natural_depression_data_local(
    ...
    if state and district and block:
        layer_name_base = f"natural_depression_{valid_gee_text(str(district).strip().lower())}_{valid_gee_text(str(block).strip().lower())}"
        ...
        layer_name_base = f"{asset_suffix}_natural_depression".lower()
    ...
    output_raster_path = build_output_raster_path(
        ...
        output_base_dir=NATURAL_DEPRESSION_OUTPUT_BASE_DIR,
        state=state,
        district=district,
        block=block,
    )
    ...
    clipped_raster_path = clip_raster_with_roi(
        ...
        raster_path=NATURAL_DEPRESSION_PAN_INDIA_LOCAL_PATH,
