...
CONDA_ENV_NAME="${CONDA_ENV_NAME:-corestackenv}"
...
function install_postgres() {
    if command -v psql >/dev/null 2>&1; then
        ...
        apt-get update
        apt-get install -y postgresql postgresql-contrib postgis libpq-dev
    fi
    ...
    su -c "psql -c \"CREATE USER $POSTGRES_USER WITH PASSWORD '$POSTGRES_PASSWORD';\" " postgres 2>/dev/null || true
    su -c "psql -c \"ALTER USER $POSTGRES_USER WITH PASSWORD '$POSTGRES_PASSWORD';\"" postgres
    su -c "psql -c \"CREATE DATABASE $POSTGRES_DB OWNER $POSTGRES_USER;\" " postgres 2>/dev/null || true
    su -c "psql -c \"ALTER USER $POSTGRES_USER WITH SUPERUSER;\"" postgres
...
function install_geoserver() {
    ...
    if [ ! -f "$tomcat_dir/bin/startup.sh" ]; then
        ...
        wget -qL \
            ...
            -O "$tomcat_tar"
        mkdir -p "$tomcat_dir"
        tar xzf "$tomcat_tar" -C "$tomcat_dir" --strip-components=1
        rm -f "$tomcat_tar"
    ...
    cp "$geoserver_war_dir/geoserver.war" "$tomcat_dir/webapps/"
    ...
    if ! geoserver_deployed; then
        ...
        echo "Check deployment logs: tail -20 $tomcat_dir/logs/catalina.out"
        return 1
    fi
...
function generate_env_file() {
    ...
    chown "${USER:-root}:${USER:-root}" "$env_file" 2>/dev/null || true
    chmod 640 "$env_file"
...
function download_admin_boundary_data() {
    ...
    # Use py7zr (Python) instead of p7zip-full — not available on all systems
